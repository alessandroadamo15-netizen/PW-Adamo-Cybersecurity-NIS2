...
Sono state quindi introdotte strutture dati per gestire:
...
- `06_query_supporto_profilo.sql` → query di analisi  
- `07_profili_acn.sql` → gestione profili ACN  
...
A partire da questa base, vengono considerate le subcategorie del Framework, a cui sono associati:
...
Lo schema è stato aggiornato per includere la gestione dei profili Target e Attuale.

Il sistema consente di mappare i livelli di maturità e valutare la postura di sicurezza attraverso il livello di copertura dei controlli, utilizzando una soglia di riferimento per la valutazione della conformità.
